...
stmt::Ptr loop::convert_to_ast_impl(loop_info &li, dominator_analysis &dta_, std::vector<std::pair<std::shared_ptr<basic_block>, stmt_block::Ptr>> &return_blocks, std::unordered_set<std::shared_ptr<basic_block>> &visited) {
        ...
        if (isa<label_stmt>(bb->parent)) {
            for (auto subloop: subloops) {
                if (subloop->header_block->parent == bb->parent) {
                    ...
                    ast->stmts.push_back(subloop->convert_to_ast_impl(li, dta_, loop_out_blocks, visited));
...
// We keep a map of parent of each ast node
std::map<stmt_block::Ptr, stmt_block::Ptr> ast_parent_map_global;
stmt_block::Ptr loop_info::convert_to_ast(stmt_block::Ptr ast) {
    // create a stmt_block to return the final converted AST block
    stmt_block::Ptr return_ast = std::make_shared<stmt_block>();
    
    // worklist to handle processing of each basic block, here the
    // pair represents basic block to processed and stmt_block in
    // which the processed AST node should be inserted. 
    // The worklist should be a 
    // std::pair <basic_block_to_be_processed, destination_ast_node>)
    std::deque<std::pair<std::shared_ptr<basic_block>, stmt_block::Ptr>> worklist;
    // keep list of already visited basic blocks
    std::unordered_set<std::shared_ptr<basic_block>> visited;
    worklist.push_back({dta.cfg_[0], return_ast});
    // start with cfg_[0] as that is the root of the CFG
    ...
    while (worklist.size()) {
        ...
        // whenever we encounter a label statement, we know
        // that next blocks are part of a loop or loop nest,
        // and the label is the loop header. So process 
        // them recursively.
        if (isa<label_stmt>(bb->parent)) {
            // Try to find the loop in the header->loop map derived
            // from loop info analysis.
            auto loop_find_res = bb_loop_map.find(bb->id);
            if (loop_find_res != bb_loop_map.end()) {
                auto loop = loop_find_res->second;

                std::cerr << "found outerloop\n";
                std::vector<std::pair<std::shared_ptr<basic_block>, stmt_block::Ptr>> loop_out_blocks;
                // push the resulting processed loop in while_stmt form into the ast stmt node.
                ast->stmts.push_back(loop->convert_to_ast_impl(*this, dta, loop_out_blocks, visited));
                loop->structured_ast_loop = to<while_stmt>(ast->stmts.back());

                // process the loop exit blocks, or the blocks that go out of the loop
                // and should be processed by it's parent loop. If the loop out block
                // provides the ast node into which it should be inserted, insert there
                // or else insert in the current ast node.
                for (auto block: loop_out_blocks) {
                    worklist.push_back({block.first, block.second ? block.second : ast});
                }
            ...
            // Since this function processes only the top level
            // blocks of the CFG, if any if_stmt is a part of a
            // loop it should be processed by the recursive call
            // to process the loop above and thus we can skip it.
            // TODO: ideally we should use visited map to check
            // this, but it creates some bugs in codegen as of now.
            ...
            // push the then branch onto worklist.
            if (bb->then_branch) {
            ...
            // push the if branch onto worklist
            if (bb->else_branch) {
            ...
            // if we see exit blocks, check if any of them are the unique
            // exit block of a loop. If yes, insert break statements in the
            // ast node.
            if (bb->is_exit_block) {
                for (auto loop: loops) {
            ...
            // if the basic block has no successor, don't process further.            
            if (!bb->successor.size()) 
                continue;

            // if this block has already been processed, don't
            // process further.
            if (visited.count(bb->successor[0]))
                continue;
            
            visited.insert(bb->successor[0]);
            // if this was an exit block, it's successor should be added in it's
            // parent ast node.
            if (bb->is_exit_block)
